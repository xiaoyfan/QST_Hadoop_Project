...
public class MonthsRemain {
	public static class Map extends Mapper<LongWritable,Text,Text,Text>{
		public void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
			...
			String filename = filesplit.getPath().getParent().getParent().getName();
			Configuration conf = context.getConfiguration();
			String pathm = conf.get("month1");
			String pathn = conf.get("month2");
	...
	public static class Reduce extends Reducer<Text,Text,Text,Text>{
		...
		public void cleanup(Reducer<Text, Text, Text, Text>.Context context)
			...
			Double result;
			result = count / sum ;
			context.write(new Text("Months remain"), new Text(result+" "+count+" "+sum));
		}
	}
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        Job job = Job.getInstance(conf,"yangxi_pro3_2");
        job.setJarByClass(MonthsRemain.class);
